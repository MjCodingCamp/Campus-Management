                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (i in snapshot.children) {
                        ...
                        if (userId == uid) {
                            ...
                            lReference.child(uYear).child(uClass).child("Notes").addListenerForSingleValueEvent(object : ValueEventListener {
                                override fun onDataChange(snapshot: DataSnapshot) {
                                    for (i in snapshot.children)
                                    {
                                        if (i.key != "ID")
                                    ...
                                    recyclerView.adapter = activity?.let { SecondAdapter(it,link,name,subject,mDate,"Notes") }
                                }
