...
class AddNotesFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val btnClick:ImageView = view.findViewById(R.id.add_notes)
    ...
    private fun uploadNotes() {
        ...
        val textView:TextView = mView.findViewById(R.id.title_Text)
        textView.text = "Notes Management"
        ...
        mEditText.hint = "Enter File Name"
    ...
    private fun uploadFile() {
        ...
        {
            val mReference = storageReference.reference.child("Notes/$sYear/$sBranch/"+"${mEditText.text}-(${System.currentTimeMillis()})"+".pdf")
        ...
        }else
        {
            ...
            {
                Toast.makeText(mContext,"Please Enter File Name",Toast.LENGTH_LONG).show()
                pDialog.dismiss()
                uploadNotes()
            } else
            {
    ...
    private fun updateData(currentUID: String, uri: Uri?, teacherName: String) {
        var uploadID = 0
        databaseReference.getReference("Login/Teacher").child("$currentUID/Notes-History").addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (i in snapshot.children)
                    {
                        if (i.key == "ID")
                        {
                            ...
                            databaseReference.getReference("Login/Teacher").child("$currentUID/Notes-History/ID").setValue(uploadID)
                            databaseReference.getReference("Login/Teacher").child("$currentUID/Notes-History/$uploadID").setValue(data)
                            databaseReference.getReference("Links/$sYear/$sBranch/Notes/ID").addListenerForSingleValueEvent(
                                object : ValueEventListener {
                                    override fun onDataChange(snapshot: DataSnapshot) {
                                        var id = snapshot.value.toString().toInt()
                                        id--
                                        databaseReference.getReference("Links/$sYear/$sBranch/Notes/ID").setValue(id)
                                        databaseReference.getReference("Links/$sYear/$sBranch/Notes/$id").setValue(data)
                                    }
                                    override fun onCancelled(error: DatabaseError) {}
                                })
                        }
